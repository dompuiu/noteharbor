...
test('surfaces native string rejections verbatim instead of the generic fallback', async () => {
  const repository: ViewerRepository = {
    loadDataset: () => Promise.resolve(dataset),
    importArchive: () => Promise.reject('Unable to read file.'),
    deleteCollection: () => Promise.resolve(dataset),
    setDefaultCollection: () => Promise.resolve(dataset),
    deleteImportedDataset: () => Promise.resolve(null),
  };
  ...
  await ReactTestRenderer.act(async () => {
    await probe.state?.importArchive('/a.zip');
    await Promise.resolve();
  });

  expect(probe.state?.error).toBe('Unable to read file.');
});
